export interface Product {
  id: number;
  name: string;
  price: string;
  category: 'sticker' | 'keychain' | 'set' | 'print';
  fandom?: 'Original' | 'Evangelion' | 'Pokemon' | 'Genshin Impact' | 'Other';
  image: string;
  description: string;
  inStock: boolean;
}

export const products: Product[] = [
  {
    id: 1,
    name: 'Rei Ayanami (Evangelion)',
    price: '$5.00',
    category: 'print',
    fandom: 'Evangelion',
    image: 'https://images.unsplash.com/photo-1541963463532-d68292c34b19?q=80&w=1080&auto=format&fit=crop', // Placeholder
    description: 'Авторский принт с Рей Аянами. Нежный и атмосферный арт, вдохновленный классикой аниме.',
    inStock: true,
  },
  {
    id: 2,
    name: 'Moonlight Cat Keychain',
    price: '$8.00',
    category: 'keychain',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1760733672584-475e728f09c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxrYXdhaWklMjBrZXljaGFpbiUyMHBhc3RlbHxlbnwxfHx8fDE3NjYzMzY5NTV8MA&ixlib=rb-4.1.0&q=80&w=1080',
    description: 'Милый акриловый брелок со спящим котиком под луной. Идет в комплекте с пастельно-розовым шармом.',
    inStock: true,
  },
  {
    id: 3,
    name: 'Pokemon Starters Set',
    price: '$10.00',
    category: 'set',
    fandom: 'Pokemon',
    image: 'https://images.unsplash.com/photo-1613771404721-1f92d799e49f?q=80&w=1080&auto=format&fit=crop', // Placeholder
    description: 'Набор стикеров с любимыми стартовиками. Яркие, водостойкие и полные ностальгии.',
    inStock: true,
  },
  {
    id: 4,
    name: 'Cloud Nine Keychain',
    price: '$7.50',
    category: 'keychain',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1628586431263-44040b966252?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxhcnRpc3QlMjB3b3Jrc3BhY2UlMjBkZXNrfGVufDF8fHx8MTc2NjMzNjk1NHww&ixlib=rb-4.1.0&q=80&w=1080',
    description: 'Пушистое облачко с мечтательной эстетикой. Выполнено из прочного прозрачного акрила.',
    inStock: true,
  },
  {
    id: 5,
    name: 'Pastel Garden Set',
    price: '$12.00',
    category: 'set',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1764344814867-d7a6916e1a37?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjdXRlJTIwc3RpY2tlcnMlMjBjb2xsZWN0aW9ufGVufDF8fHx8MTc2NjMzNjk1NXww&ixlib=rb-4.1.0&q=80&w=1080',
    description: 'Коллекция из 5 ботанических стикеров в мягких пастельных тонах. Каждый нарисован вручную.',
    inStock: false,
  },
  {
    id: 6,
    name: 'Starlight Bear Keychain',
    price: '$8.50',
    category: 'keychain',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1760733672584-475e728f09c5?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxrYXdhaWklMjBrZXljaGFpbiUyMHBhc3RlbHxlbnwxfHx8fDE3NjYzMzY5NTV8MA&ixlib=rb-4.1.0&q=80&w=1080',
    description: 'Очаровательный мишка, держащий крошечную звездочку. Идеальный компаньон для твоих ключей или сумки.',
    inStock: true,
  },
  {
    id: 7,
    name: 'Zarya Mandarin Collab',
    price: '$6.00',
    category: 'sticker',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1617646160236-db27e21e4efe?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxoYW5kbWFkZSUyMGNyYWZ0JTIwYXJ0fGVufDF8fHx8MTc2NjMzNjk1Nnww&ixlib=rb-4.1.0&q=80&w=1080',
    description: 'Специальный стикер с маскоткой маркета Заря — Мандаринкой. Результат теплой и солнечной коллаборации.',
    inStock: true,
  },
  {
    id: 8,
    name: 'Mini Moments Set',
    price: '$15.00',
    category: 'set',
    fandom: 'Original',
    image: 'https://images.unsplash.com/photo-1764344814867-d7a6916e1a37?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjdXRlJTIwc3RpY2tlcnMlMjBjb2xsZWN0aW9ufGVufDF8fHx8MTc2NjMzNjk1NXww&ixlib=rb-4.1.0&q=80&w=1080',
    description: '8 крошечных стикеров, прославляющих повседневные моменты. От чашек кофе до уютных пледов.',
    inStock: false,
  },
];
